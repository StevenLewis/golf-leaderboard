...
describe('Competition', () => {
    test('It returns the competition results', () => {
        ...
        expect(competition.results.all()).toHaveLength(4)
    })
    ...
    test('It can order the results by nett and countback', () => {
        ...
        competition.withResults(results).orderResultsByScore()

        expect(competition.results.items[0].id).toEqual('1')
        expect(competition.results.items[1].id).toEqual('4')
        expect(competition.results.items[2].id).toEqual('2')
        expect(competition.results.items[3].id).toEqual('5')
        expect(competition.results.items[4].id).toEqual('3')
    })
})
